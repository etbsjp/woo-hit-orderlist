...
/* タクソノミーが存在しない場合に追加
/*-------------------------------------------*/
function whol_ensure_product_tag($tag_name, $tag_slug) {
    // タグが存在するか確認
...
/* エクセル出力
/*-------------------------------------------*/
function whol_enqueue_xlsx_export( $wid, $fname ) {
...
/*-------------------------------------------*/
function whol_enqueue_enhanced_select(){
 ...
 *
 * ★ フック名 `wp_ajax_etbs_woo_sendmailhit` と nonce の action 文字列
 * `etbs_woo_sendmailhit` は据え置き。プラグイン読み込み→テーマ functions.php 読み込みの順で
 * プラグイン側のコールバックが必ず先に登録され、check_ajax_referer() が nonce 不一致で
 * 即 403 を返すため、テーマ側の旧ハンドラー（存在する場合）には絶対に到達しない。
 *
 * @return void 送信結果を echo し wp_die() で応答する。
 */
function whol_send_order_note(){
...
// 実物（社内で保全している検証用の旧コピー）で実測済み。同一優先度では登録順で決まり、
// wp-settings.php の読み込み順は「通常プラグイン（589行目）→ テーマ functions.php（738行目）」
// なので、優先度が同じである限りプラグイン側のこのコールバックが必ず先に登録される。
// ここを既定のままにしているのは実測に基づく判断であり、書き忘れではない。
